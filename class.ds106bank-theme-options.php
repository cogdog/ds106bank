  ...
  <br /><input type="button" class="upload_image_button button-primary" name="_ds106banker_button' . $id .'" id="_ds106banker_button' . $id .'" data-options_id="' . $id  . '" data-uploader_title="Set ' .  bank106_option( 'thingname' ) . ' Thumbnail" data-uploader_button_text="Select Thumbnail" value="Set/Change Thumbnail" />
	...
	public function validate_settings( $input ) {
		...
		if ( ! isset( $input['reset_theme'] ) ) {
			$options = get_option( 'bank106_options' );
				
			// has the thing name changed? If so we need to update the taxonmy terms
			if ( $input['thingname'] !=  bank106_option( 'thingname' )  ) {
				bank106_update_tax( bank106_option( 'thingname' ), $input['thingname'] );
			}
			
			// has thumbnail sizes changed? If so, update option for thumbnail sizes
			if ( $input['thumb_w'] !=   bank106_option( 'thumb_w' )  ) {
				update_option('thumbnail_size_w', $input['thumb_w']);
			}
			
			if ( $input['thumb_h'] !=  bank106_option( 'thumb_h' )  ) {
...
$theme_options = new bank106_Theme_Options();

function bank106_option( $option ) {
	$options = get_option( 'ds106banker_options' );
