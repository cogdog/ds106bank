				...
				<?php if ( $my_show_ex != 'none'): // show at least examples and/or tutorials ?>
					...
					<div class="clearfix row hilite">
						...
						<?php if ( $my_show_ex != 'tut' ) :?>
							...
							<h3>Respond to This <?php echo bank106_option( 'thingname' )?></h3>

							<p>After you respond to this <?php echo lcfirst(bank106_option( 'thingname' ))?> please share a link to it and a description so it can be added to the ones listed below.
						...
						<?php if ( $my_show_ex != 'ex' ) : ?>
							...
							<h3><?php echo bank106_option('helpthingname')?>s for this <?php echo bank106_option( 'thingname' )?></h3>
								<p>Have you created a helpful <?php echo lcfirst(bank106_option('helpthingname'))?> or do you know of one that might help others with this <?php echo lcfirst(bank106_option( 'thingname' ))?>?

								<?php if ( bank106_option('helpthingname') == 'internal' ):?>
								If you are writing on a blog connected to this site include the tag <strong><?php echo $my_tutorial_tag;?></strong> when writing a post on your own blog. A link to your <?php echo lcfirst(bank106_option('helpthingname'))?> A should be automatically added below within an hour.  <br /><br />Or you

								<?php elseif ( bank106_option('helpthingname') == 'external' ):?>
								If you are writing to a site that is connected to  <a href="<?php echo bank106_option('syndication_site_url')?>"><?php echo bank106_option('syndication_site_name')?></a> include the following yags when writing a post on your own blog. (You must use BOTH!):  <strong><?php echo bank106_option( 'extra_tag' ) . ', ' .  $my_tutorial_tag;?></strong> Then a link to your <?php echo lcfirst(bank106_option('helpthingname'))?> will be automatically added within an hour to the ones below. <br /><br />Or you

								<?php else:?>
								You
								<?php endif?>

								can share a <?php echo lcfirst(bank106_option('helpthingname'))?> if it is available at a public URL<?php if ( bank106_option( 'new_example_status' ) == 'draft') echo ' (it will appear below after moderator approval)'?>.</p><?php echo $signin?><p class="text-center"><a href="<?php echo site_url(); ?>/<?php echo bank106_option( 'example_form_page' );?>/?aid=<?php echo $my_id?>&typ=tut" class="btn btn-primary<?php echo $extra_btn_class?>"><span class="glyphicon glyphicon-hand-right" aria-hidden="true"></span> Add a <?php echo bank106_option('helpthingname')?></a>
